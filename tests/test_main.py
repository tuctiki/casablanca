...
def test_cache_skips_if_folder_exists(mock_move, mock_cat, mock_summary, mock_transcript, mock_get_video_metadata):
    ...
    exit_code, log_output = run_main([video_url], caplog)
...
def test_force_processes_if_folder_exists(mock_move, mock_cat, mock_summary, mock_transcript, mock_get_video_metadata, caplog):
    ...
    exit_code, log_output = run_main([video_url, "--force"], caplog)
...
@patch('casablanca.main.get_video_category')
@patch('casablanca.main.move_to_obsidian')
def test_custom_categories_option(mock_move, mock_cat, mock_summary, mock_transcript, mock_get_video_metadata, caplog):
    ...
    exit_code, log_output = run_main([video_url, "--categories", custom_categories], caplog)
...
def test_invalid_url_exits_with_error(mock_get_video_metadata, caplog):
    video_url = "invalid_url"
    exit_code, log_output = run_main([video_url], caplog)
    assert exit_code == 1
    mock_get_video_metadata.assert_called_once_with(video_url)
    assert "Failed to get video metadata." in log_output

@patch('casablanca.main.get_video_metadata')
@patch('casablanca.main.get_transcript', return_value=None)
@patch('casablanca.main.get_video_category', return_value="Finance")
def test_transcript_failure_exits_with_error(mock_get_video_category, mock_get_transcript, mock_get_video_metadata, caplog):
    video_url = "https://www.youtube.com/watch?v=test_video_id"
    ...
    assert "Failed to fetch transcript. Exiting summarization process." in log_output
...
@patch('casablanca.main.move_to_obsidian')
def test_non_finance_video_skips_summarization(mock_move, mock_cat, mock_summary, mock_transcript, mock_get_video_metadata, caplog):
    video_url = "https://www.youtube.com/watch?v=test_video_id"
    mock_get_video_metadata.return_value = {"title": "Test Title", "description": "Test Description", "publishedAt": "2023-10-26T12:00:00Z"}
    exit_code, log_output = run_main([video_url], caplog)
    assert exit_code == 0
    ...
    mock_summary.assert_not_called()
    mock_move.assert_not_called()
    assert "Video is not finance-related (Sports). Skipping transcript fetching and summarization." in log_output

@patch('casablanca.main.get_video_metadata', return_value=None)
def test_log_level_option(mock_get_video_metadata, caplog):
    video_url = "https://www.youtube.com/watch?v=test_video_id"
    exit_code, log_output = run_main([video_url, "--log-level", "DEBUG"], caplog)
    assert exit_code == 1
    mock_get_video_metadata.assert_called_once_with(video_url)
    assert "Failed to get video metadata." in log_output
...
@patch('casablanca.main.summarize_content', return_value="This is a test summary.")
@patch('casablanca.main.get_video_category', return_value="Finance")
def test_move_to_obsidian_logs_warning_if_path_not_set(mock_get_video_category, mock_summarize_content, mock_get_transcript, mock_get_video_metadata, caplog):
    video_url = "https://www.youtube.com/watch?v=test_video_id"
    mock_get_video_metadata.return_value = {"title": "Test Title", "description": "Test Description", "publishedAt": "2023-10-26T12:00:00Z"}
    exit_code, log_output = run_main([video_url], caplog)
    assert exit_code == 0
    assert "OBSIDIAN_VAULT_PATH not set. Skipping move to Obsidian." in log_output